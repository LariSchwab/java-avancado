...
async function apagar(evento) {
    // evento é uma variável que fica dosponível quando ocorre o clique do botão, poderia ser qualquer nome no lugar de evento
    // utilizaremos o evento para saber qual o botão que ocorreu o clique
    // obter o botão que ocorrei o click evento
const botaoClique = evento.target;

// data - são atributos(variáveis) colocadas no HTML, para podermos ter acesso ao javascript
...
async function consultarEmpresas() {
    ...
    let url = `${urlAPI}/api/v1/empresa`
    // fetch vai realizar a requisição, na variável resposta teremos os dados do response como: status, response em si 
    // (dados que back-end retornou)
    ...
    // Obter o response da reuisição, que neste cenário será uma lista de objetos
